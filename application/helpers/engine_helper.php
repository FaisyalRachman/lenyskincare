<?php 
    ...
    function no_telp(){
        $ci = & get_instance();
        $fav = $ci->db->query("SELECT no_telp FROM identitas ORDER BY id_identitas DESC LIMIT 1")->row_array();
        return $fav['no_telp'];
    }
    function email(){
        $ci = & get_instance();
        $fav = $ci->db->query("SELECT email FROM identitas ORDER BY id_identitas DESC LIMIT 1")->row_array();
        return $fav['email'];
    }
