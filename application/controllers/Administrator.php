...
class Administrator extends CI_Controller
{
    ...
    function identitaswebsite()
    {
        ...
        if (isset($_POST['submit'])) {
            ...
            $config['allowed_types'] = 'mp4|gif|jpg|png|ico';
            $config['max_size'] = '1222500'; // kb
    ...
    function aboutbracatour()
    {
        ...
        if (isset($_POST['submit'])) {
            ...
            if ($hasil['file_name'] == '') {
                $data = array(
                    'isi_halaman' => $this->db->escape_str($this->input->post('aboubracatour1')),
                    'isi_halaman1' => $this->db->escape_str($this->input->post('aboutbracatour2'))
            ...
            $where = array('id_halaman' => 1);
            $this->model_app->update('halamanstatis', $data, $where);

            redirect($this->uri->segment(1) . '/aboutbracatour');
        } else {
            $proses = $this->model_app->edit('halamanstatis', array('id_halaman' => 1))->row_array();
            $data = array('record' => $proses);
            $this->template->load('administrator/template', 'administrator/mod_aboutbraca/view_aboutbraca', $data);
        }
    }
    
 function identitasclient()
    {
        cek_session_akses('identitasclient', $this->session->id_session);
        if (isset($_POST['submit'])) {
            $config['upload_path'] = 'asset/images/';
            ...
            redirect($this->uri->segment(1) . '/identitasclient');
        } else {
            $proses = $this->model_app->edit('identitas', array('id_identitas' => 1))->row_array();
            $data = array('record' => $proses);
            $this->template->load('administrator/template', 'administrator/mod_identitas/view_identitas', $data);
    ...
    function inputdeliveryorder()
    {
      //  cek_session_akses('inputdeliveryorder', $this->session->id_session);
        if (isset($_POST['submit'])) {
            $config['upload_path'] = 'asset/images/';
            $config['allowed_types'] = 'gif|jpg|png|ico';
            $config['max_size'] = '500'; // kb
            ...
            $this->model_app->update('productcek', $data, $where);

            redirect($this->uri->segment(1) . '/inputdeliveryorder');
        } else {
           $proses = $this->model_app->edit('productcek', array('id' => 1))->row_array();
           $data = array('record' => $proses);
           // $data['record'] = $this->model_app->view('productcek');
            $this->template->load('administrator/template', 'administrator/mod_productcek/view_icproduct', $data);
        }
    }
    function inputstock()
    {
        $this->config->set_item('global_xss_filtering', false);
        //cek_session_akses('inputstock', $this->session->id_session);
        if (isset($_POST['submit'])) {
            $config['upload_path'] = 'asset/images/';
            $config['allowed_types'] = 'gif|jpg|png|ico';
            $config['max_size'] = '500'; // kb
            ...
            redirect($this->uri->segment(1) . '/inputstock');
        } else {
           $proses = $this->model_app->edit('productcek', array('id' => 2))->row_array();
           $data = array('record' => $proses);
           // $data['record'] = $this->model_app->view('productcek');
            $this->template->load('administrator/template', 'administrator/mod_productcek/view_icheckstock', $data);
        }
    }
    function viewdeliveryorder()
    {
      //  cek_session_akses('viewdeliveryorder', $this->session->id_session);
        if (isset($_POST['submit'])) {
            $config['upload_path'] = 'asset/images/';
            $config['allowed_types'] = 'gif|jpg|png|ico';
            $config['max_size'] = '500'; // kb
            ...
            redirect($this->uri->segment(1) . '/viewdeliveryorder');
        } else {
           $proses = $this->model_app->edit('productcek', array('id' => 1))->row_array();
            $data = array('record' => $proses);
           // $data['record'] = $this->model_app->view('productcek');
            $this->template->load('administrator/template', 'administrator/mod_productcek/view_cdoproduct', $data);
        }
    }
    function viewstockitem()
    {
       // cek_session_akses('viewstockitem', $this->session->id_session);
        if (isset($_POST['submit'])) {
            $config['upload_path'] = 'asset/images/';
            $config['allowed_types'] = 'gif|jpg|png|ico';
            $config['max_size'] = '500'; // kb
            ...
            redirect($this->uri->segment(1) . '/viewstockitem');
        } else {
           $proses = $this->model_app->edit('productcek', array('id' => 2))->row_array();
            $data = array('record' => $proses);
           // $data['record'] = $this->model_app->view('productcek');
            $this->template->load('administrator/template', 'administrator/mod_productcek/view_cstockitem', $data);
        }
    }
    ...
    function whybraca()
    {
       // cek_session_akses('listberita', $this->session->id_session);
        if ($this->session->level == 'admin' || $this->session->level == 'staf') {
            $data['record'] = $this->model_app->view_ordering('whybraca', 'id_whybraca', 'DESC');
        } else {
            $data['record'] = $this->model_app->view_where_ordering('whybraca', array('username' => $this->session->username), 'id_whybraca', 'DESC');
        }
        $data['rss'] = $this->model_utama->view_joinn('whybraca', 'users', 'kategori', 'username', 'id_kategori', 'id_whybraca', 'DESC', 0, 10);
        $data['iden'] = $this->model_utama->view_where('identitas', array('id_identitas' => 1))->row_array();
        $this->load->view('administrator/rss', $data);
        $this->template->load('administrator/template', 'administrator/mod_whybraca/view_whybraca', $data);
    }
       ...
       // cek_session_akses('listberita', $this->session->id_session);
        if (isset($_POST['submit'])) {
          ...
          var_dump($array);
            ...
            $data['record'] = $this->model_app->view_where_ordering('kategori',array('sidebar' => '2'), 'id_kategori', 'DESC');
...
function listproduk()
    {
      // cek_session_akses('listproduk', $this->session->id_session);
        if ($this->session->level == 'admin' || $this->session->level == 'staf') {
            $data['record'] = $this->model_app->view_ordering('produk', 'id_produk', 'DESC');
        } else {
            $data['record'] = $this->model_app->view_where_ordering('produk', array('username' => $this->session->username), 'id_produk', 'DESC');
        }
        $data['rss'] = $this->model_utama->view_joinn('produk', 'users', 'subkategori', 'username', 'id_subkategori', 'id_produk', 'DESC', 0, 10);
        $data['iden'] = $this->model_utama->view_where('identitas', array('id_identitas' => 1))->row_array();
        $this->load->view('administrator/rss', $data);
        $this->template->load('administrator/template', 'administrator/mod_produk/view_produk', $data);
    }

    function tambah_listproduk()
    {
       // cek_session_akses('listproduk', $this->session->id_session);
        if (isset($_POST['submit'])) {
            $config['upload_path'] = 'asset/foto_produk/';
            $config['allowed_types'] = 'pdf|doc|docx|xls|xlsx|gif|jpg|png|JPG|JPEG|jpeg|PNG';
            $config['max_size'] = '7000'; // kb
            $this->load->library('upload', $config);
            $this->upload->do_upload('userfile');
           // $this->upload->do_upload('filesheet');
            $hasil = $this->upload->data();

            if (isset($_FILES['userfile1']) && $_FILES['userfile1']['name'] != ''){
                $file1 = $this->upload->do_upload('userfile1');
                $hasil1 = $this->upload->data();
                $file1new = $hasil1['file_name'];
            }
                
            if (isset($_FILES['userfile2']) && $_FILES['userfile2']['name'] != ''){
                $file2 = $this->upload->do_upload('userfile2');
                $hasil2 = $this->upload->data();
                $file2new = $hasil2['file_name'];
            }   
            $config['source_image'] = 'asset/foto_produk/' . $hasil['file_name'];
         //   $config['source_image'] = 'asset/foto_produk/' . $hasil['file_name'];
            $config['wm_text'] = '';
            $config['wm_type'] = 'text';
            $config['wm_font_path'] = './system/fonts/texb.ttf';
            ...
            redirect($this->uri->segment(1) . '/listproduk');
        } else {
            $data['tag'] = $this->model_app->view_ordering('tag', 'id_tag', 'DESC');
            $data['record'] = $this->model_app->view_join_one('subkategori','kategori','id_kategori', 'id_subkategori', 'DESC');
            $this->template->load('administrator/template', 'administrator/mod_produk/view_produk_tambah', $data);
        }
    }

    function edit_listproduk()
    {
        //cek_session_akses('listproduk', $this->session->id_session);
        $id = $this->uri->segment(3);
        if (isset($_POST['submit'])) {
            $config['upload_path'] = 'asset/foto_produk/';
            $config['allowed_types'] = 'pdf|doc|docx|xls|xlsx|gif|jpg|png|JPG|JPEG|jpeg|PNG';
            $config['max_size'] = '1100000'; // kb
            $this->load->library('upload', $config);
            $this->upload->do_upload('userfile');
            $hasil = $this->upload->data();
$efile1 = $this->input->post('euserfile1');
$efile2 = $this->input->post('euserfile2');
            if (isset($_FILES['userfile1']) && $_FILES['userfile1']['name'] != ''){
                $file1 = $this->upload->do_upload('userfile1');
                $hasil1 = $this->upload->data();
                $file1new = $hasil1['file_name'];
            ...
            } elseif($_FILES['userfile1']['name'] != '' && $_FILES['userfile2']['name'] == ''){
                $data = array(
                    'id_subkategori' => $this->db->escape_str($this->input->post('a')),
                    'username' => $this->session->username,
                    'judul' => $this->db->escape_str($this->input->post('b')),
            ...
            $tag = $this->model_app->view_ordering('tag', 'id_tag', 'DESC');
           // $record = $this->model_app->view_ordering('kategori', 'id_kategori', 'DESC');
            $record = $this->model_app->view_join_one('subkategori','kategori','id_kategori', 'id_subkategori', 'DESC');
          
            if ($this->session->level == 'admin') {
                $proses = $this->model_app->edit('produk', array('id_produk' => $id))->row_array();
            } else {
                $proses = $this->model_app->edit('produk', array('id_produk' => $id, 'username' => $this->session->username))->row_array();
            }
            $data = array('rows' => $proses, 'tag' => $tag, 'record' => $record);
    ...
    function delete_listproduk()
    {
        cek_session_akses('listproduk', $this->session->id_session);
        if ($this->session->level == 'admin') {
            $id = array('id_produk' => $this->uri->segment(3));
        } else {
            $id = array('id_produk' => $this->uri->segment(3), 'username' => $this->session->username);
        }
...
// end produk
     ...
     function kategoriproduct()
    {
      ...
      if ($this->session->level == 'admin' || $this->session->level == 'supervisor' || $this->session->level == 'staf') {
        ...
        $this->template->load('administrator/template', 'administrator/mod_kategoriproduct/view_kategoriproduct', $data);
    }
...
function subkategoriproduct()
    ...
    function tambah_kategoriproduct()
    {
          if (isset($_POST['submit'])) {
            $config['upload_path'] = 'asset/foto_berita/';
            $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG|jpeg|PNG|swf';
            ...
            $this->model_app->insert('kategoriproduct', $data);
            redirect($this->uri->segment(1) . '/kategoriproduct');
        } else {
            $this->template->load('administrator/template', 'administrator/mod_kategoriproduct/view_kategoriproduct_tambah');
        }
    }
    function tambah_subkategori()
    {
      //  cek_session_akses('subkategori', $this->session->id_session);
        if (isset($_POST['submit'])) {
            ...
            $this->upload->do_upload('d');
            $hasil = $this->upload->data();
            $string = trim(preg_replace('/\s\s+/', ' ', $this->input->post('f')));
            if ($hasil['file_name'] == '') {
                $data = array(
                    'nama_subkategori' => $this->db->escape_str($this->input->post('a')),
            ...
            $data['recordkat'] = $this->model_app->view_where('kategori', array('aktif' => 'Y','sidebar' => '2'));
            $this->template->load('administrator/template', 'administrator/mod_subkategori/view_subkategori_tambah',$data);
        }
    }
    function tambah_subkategoriproduct()
    {
         if (isset($_POST['submit'])) {
            $config['upload_path'] = 'asset/foto_produk/';
            $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG|jpeg|PNG|swf';
            ...
            $this->model_app->insert('subkategoriproduct', $data);
            redirect($this->uri->segment(1) . '/subkategoriproduct');
        } else {
            $data['recordkat'] = $this->model_app->view_where('kategoriproduct', array('aktif' => 'Y','sidebar' => '2'));
            $this->template->load('administrator/template', 'administrator/mod_subkategoriproduct/view_subkategoriproduct_tambah',$data);
        }
    }
     ...
     function edit_kategoriproduct()
    {
        ...
        if (isset($_POST['submit'])) {
            ...
            $data = array('rows' => $proses);
            $this->template->load('administrator/template', 'administrator/mod_kategoriproduct/view_kategoriproduct_edit', $data);
        }
    }
    function edit_subkategori()
    {
       // cek_session_akses('subkategori', $this->session->id_session);
        ...
        if (isset($_POST['submit'])) {
            ...
            $data = array('rows' => $proses);
            $data['recordkat'] = $this->model_app->view_where('kategori', array('aktif' => 'Y','sidebar' => '2'));
            $this->template->load('administrator/template', 'administrator/mod_subkategori/view_subkategori_edit', $data);
        }
    }
     function edit_subkategoriproduct()
    {
        $id = $this->uri->segment(3);
        if (isset($_POST['submit'])) {
            $config['upload_path'] = 'asset/foto_produk/';
            ...
            $where = array('id_subkategori' => $this->input->post('id'));
            $this->model_app->update('subkategoriproduct', $data, $where);
            redirect($this->uri->segment(1) . '/subkategoriproduct');
        } else {
            if ($this->session->level == 'admin') {
                $proses = $this->model_app->edit('subkategoriproduct', array('id_subkategori' => $id))->row_array();
            } else {
                $proses = $this->model_app->edit('subkategoriproduct', array('id_subkategori' => $id, 'username' => $this->session->username))->row_array();
            }
            $data = array('rows' => $proses);
            $data['recordkat'] = $this->model_app->view_where('kategoriproduct', array('sidebar' => '2'));
            $this->template->load('administrator/template', 'administrator/mod_subkategoriproduct/view_subkategoriproduct_edit', $data);
        }
    }
    function delete_kategoritreatment()
    {
     ...
     function delete_kategoriproduct()
    {
        if ($this->session->level == 'admin') {
            $id = array('id_kategori' => $this->uri->segment(3));
        } else {
        ...
        $this->model_app->delete('kategoriproduct', $id);
        redirect($this->uri->segment(1) . '/kategoriproduct');
    }
    function delete_subkategori()
    {
        cek_session_akses('kategoriberita', $this->session->id_session);
        if ($this->session->level == 'admin') {
            $id = array('id_subkategori' => $this->uri->segment(3));
        } else {
            $id = array('id_subkategori' => $this->uri->segment(3), 'username' => $this->session->username);
        }
        $this->model_app->delete('subkategori', $id);
        redirect($this->uri->segment(1) . '/subkategori');
    }
     function delete_subkategoriproduct()
    {
        cek_session_akses('subkategoriproduct', $this->session->id_session);
        if ($this->session->level == 'admin') {
            $id = array('id_subkategori' => $this->uri->segment(3));
        } else {
            $id = array('id_subkategori' => $this->uri->segment(3), 'username' => $this->session->username);
        }
        $this->model_app->delete('subkategoriproduct', $id);
        redirect($this->uri->segment(1) . '/subkategoriproduct');
    }
    ...
    function bukutamu()
    {
      ...
      if ($this->session->level == 'admin' || $this->session->level == 'supervisor' || $this->session->level == 'staf') {
        ...
        $this->template->load('administrator/template', 'administrator/mod_bukutamu/view_bukutamu', $data);
    }
    ...
    function tambah_albumslide()
    {
        ...
        if (isset($_POST['submit'])) {
            ...
            $this->model_app->insert('album', $data);
            redirect($this->uri->segment(1) . '/albumslide');
        } else {
            $this->template->load('administrator/template', 'administrator/mod_album/view_album_tambah');
        }
    }

    function edit_albumslide()
    {
        cek_session_akses('album', $this->session->id_session);
        $id = $this->uri->segment(3);
        if (isset($_POST['submit'])) {
    ...
    function edit_whybraca()
    {
        ...
        if (isset($_POST['submit'])) {
            ...
            $data = array('rows' => $proses);
            $this->template->load('administrator/template', 'administrator/mod_whybraca/view_editwhybraca', $data);
        }
    }
    ...
    function delete_bukutamu()
    {
        ...
        if ($this->session->level == 'admin') {
            ...
            $id = array('id_hubungi' => $this->uri->segment(3), 'username' => $this->session->username);
        }
        $this->model_app->delete('hubungi', $id);
        redirect($this->uri->segment(1) . '/bukutamu');
    }
        ...
        if (isset($_POST['submit'])) {
            ...
            if ($hasil['file_name'] == '') {
                $data = array(
                    ...
                    'username' => $this->session->username,
                    'url' => $this->input->post('b'),
                    'number' => $this->input->post('d'),
                    'gmap' => $this->input->post('e'),
    ...
    function cdo()
    {
        ...
        if (isset($_POST['submit'])) {
            ...
            $where = array('id_logo' => $this->input->post('id'));
            $this->model_app->update('logo', $datadb, $where);
            redirect($this->uri->segment(1) . '/cdo');
        } else {
            $data['record'] = $this->model_app->view('logo');
            $this->template->load('administrator/template', 'administrator/mod_productcek/view_editicproduct', $data);
        }
    }
    ...
    function tambah_manajemenuser()
    {
        ...
        if (isset($_POST['submit'])) {
            ...
            $this->session->set_flashdata('sukses', '<div class="alert alert-success">User Berhasil Ditambahkan!</div>');
    ...
    function edit_manajemenuser()
    {
        ...
        if (isset($_POST['submit'])) {
            ...
            $this->model_app->update('users', $data, $where);
            $this->session->set_flashdata('message', '<div class="alert alert-success"><center>Berhasil diperbaharui...!</center></div>');
            $mod = count($this->input->post('modul'));
            $modul = $this->input->post('modul');
            for ($i = 0; $i < $mod; $i++) {
                $datam = array(
