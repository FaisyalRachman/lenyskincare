...
public function view_ordering_limit2($table,$order,$ordering,$baris,$dari){
         $this->db->select('*');
        $this->db->order_by($order,$ordering);
        $this->db->limit($dari, $baris);
    ...
    function cari_berita($kata){
        ...
        $cari = "SELECT id_berita, username, gambar, judul, idfront, judul_seo FROM berita 
                       WHERE judul like '%".$pisah_kata."%' OR isi_berita LIKE '%".$pisah_kata."%'";
      //  $cari .= "";
                     
       $cari .= "UNION SELECT id_produk, username, gambar, judul, idfront, judul_seo FROM produk 
               WHERE judul like '%".$pisah_kata."%' OR isi_produk LIKE '%".$pisah_kata."%'";    
               
        $cari .= "UNION SELECT id_testimoni, username, gambar, source, idfront, url FROM testimoni 
               WHERE source like '%".$pisah_kata."%' OR judul LIKE '%".$pisah_kata."%'";            
        
        $cari .= "UNION SELECT id_kategori, nama_kategori, gambar_utama, deskripsi, detail, kategori_seo FROM kategori
        WHERE nama_kategori LIKE '%".$pisah_kata."%' OR deskripsi LIKE '%".$pisah_kata."%'";      
    ...
    function promo(){
        $cari = "SELECT * FROM (SELECT * FROM subkategori WHERE subkategori.aktif = 'Y' AND subkategori.recomended='Y') as t1";
      //  $cari .= "";
                     
       $cari .= " UNION SELECT * FROM (SELECT * FROM subkategoriproduct WHERE subkategoriproduct.aktif = 'Y' and subkategoriproduct.recomended='Y') as t2";    
               return $this->db->query($cari);         
    }
